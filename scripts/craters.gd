@tool
extends Node2D

const CRATER = preload("uid://d23uxg6wsi6f3")

## Scale for the offset of crater placement
const BASE_OFFSET_SCALE = 4
const MIN_GAP = 128

@export_tool_button("Regenerate", "Callable") var regenerate = _regenerate_craters

@export var crater_count:int = 5000
@export var spread:int = 100

func _regenerate_craters():
	var rng := RandomNumberGenerator.new()
	print_debug("Hi")
	for child in get_children():
		child.queue_free()
	
	var crater_poses: Array[Vector2] = []
	for i in crater_count:
		var crater:Sprite2D = CRATER.instantiate()
		crater.variant = rng.rand_weighted([.4, .4, .2])
		
		var p:Vector2
		while true:
			p = Vector2(
				rng.randi_range(-spread, spread),
				rng.randi_range(-spread, spread),
			) * BASE_OFFSET_SCALE
			
			var ok:bool = true
			for pos in crater_poses:
				if pos.distance_squared_to(p) < MIN_GAP * MIN_GAP:
					ok = false
					break
			if ok:
				break
		
		self.add_child(crater, true)
		crater.owner = self
		crater.position = p
		crater_poses.push_back(p)
