@tool
class_name ColorblindnessSimulator
extends CanvasLayer
...
func _set_filter(value:Filter) -> void:
	_filter = value
	_set_color_matrix(COLOR_MATRICES[value])
	shader_rect.visible = Filter.NONE != value


## Set the color transformation matrix in the shader
func _set_color_matrix(matrix:Basis) -> void:
	(shader_rect.material as ShaderMaterial).set_shader_parameter("color_matrix", matrix)
